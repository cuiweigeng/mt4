//------------------------------------------------------------------------------------------------+
//                                                                                                |
//                                    ivan FFCal Panel.mq4                                      |
//                                                                                                |
//------------------------------------------------------------------------------------------------+
#property copyright "Copyright @ 2017 ivan.tsui"
#property link      "email: cuiweigeng@hotmail.com" 
...
int start() {
  ...
  <event>
  <title>NAB Business Confidence</title>
  <country>AUD</country>
  <date><![CDATA[ 02-14-2017 ]]></date>
  <time><![CDATA[ 12:30am ]]></time>
  <impact><![CDATA[ Low ]]></impact>
  <forecast><![CDATA[ 2.4% ]]></forecast>
  <previous><![CDATA[ 2.1% ]]></previous>
  </event>
  */
  newsIdx = 0;
  nextNewsIdx = -1;	
  ...
  while (true) {
    ...
    if(!IsNewsCurrency(Symbol(),mainData[newsIdx][COUNTRY]))	{ //deVries
      ...
      && (StringSubstr(mainData[newsIdx][TITLE],0,4)== "Bank")) {
      skip = true;
    ...
    if (!skip) {   
      ...
      if ((StringFind(mainData[newsIdx][TITLE], "Bank Holiday", 
        StringLen(mainData[newsIdx][TITLE])-12) != -1) 
        && (minsTillNews <= 0)) {
      ...
      if (Alert_Minutes_Before != 0 
         && minsTillNews == Alert_Minutes_Before) {
...
void OutputToChart() {
  ...
  if(x1<94) {
    ...
    x2 = x1-94+95; // x2 = starts where x1 characters stop
  }
  ...
  ObjectSetText(Sponsor, "ivan.tsui SYSTEM  FFCAL HEADLINES:", TxtSize, "Verdana", FFCal_Title);

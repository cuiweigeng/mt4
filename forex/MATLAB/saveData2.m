function saveData2(H, L, C, indPrd, lookForwardLen, dim, waveletLen, waveletParam, len, file1, file2)

%% train Wavelet 
trainData = zeros(len-lookForwardLen-waveletLen, dim, dim);
verifyData = zeros(len-lookForwardLen-waveletLen, 1);
fidTrain=fopen(file1, 'wb','ieee-be'); %´ó¶Ë
...
%     maL = ma(tmpL, waveletLen, indPrd);
    ...
    stochC = stoch(tmpC(end-indCalcLen:end), length(tmpC(end-indCalcLen:end)), indPrd);

    [mid, uppr, lowr] = bollinger(tmpC(end-indCalcLen:end), indPrd, 0, 2.0);
    
    cciC = indicators([tmpH(end-indCalcLen:end), tmpL(end-indCalcLen:end), tmpC(end-indCalcLen:end)], 'cci' ,indPrd, indPrd, 0.015);
    
    
    
    
    %% remove indPrd, time start at (waveletLen+indPrd)
%     maH = maH(indPrd + 1:end);
%     maL = maL(indPrd + 1:end);
%     maC = maC(indPrd + 1:end);
%     uppr = uppr(indPrd + 1:end);
%     lowr = lowr(indPrd + 1:end);
%     stochC = stochC(indPrd + 1:end);
%     rsiC = rsiC(indPrd + 1:end);
%     cciC = cciC(indPrd + 1:end);
...
%     trainData(i, 2, :) = normalization(maL(end-lookForwardLen+1:end));
    ...
    %% valid data
    lookForwardCw = realCw(range(end)+lookForwardLen);
